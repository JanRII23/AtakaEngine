...
dx3d::VertexShaderSignature::VertexShaderSignature(const VertexShaderSignatureDesc& desc, const GraphicsResourceDesc& gDesc): GraphicsResource(gDesc), m_vsBinary(desc.vsBinary)
{
	...
	{
		auto param = params[i];
		DXGI_FORMAT format = GraphicsUtils::GetDXGIFormatFromMask(param.ComponentType, param.Mask);
		...
		currentOffset += GraphicsUtils::GetDXGIFormatSize(format);
	}
}
